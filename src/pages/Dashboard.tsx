...
import {
  CalendarDays, Briefcase, Clock, Plus, Trash2, ChevronLeft, ChevronRight, Building2, Settings
} from 'lucide-react';
  ...
  const handleCreateBusiness = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const details = Array.isArray(response?.details)
        ? response.details.map((detail: { field?: string; message?: string }) => `${detail.field ? `${detail.field}: ` : ''}${detail.message || ''}`).join(' | ')
        : '';
      showToast([response?.message || 'Error al crear negocio', details].filter(Boolean).join(' - '));
